...
xor_gate::xor_gate(size_t in_pin, nts::Tristate val, size_t sin_pin,
...
void xor_gate::setLink(std::size_t pin, nts::IComponent &other,
		std::size_t otherPin)
{
	struct link	*tmp = link_pin;
...
struct pin xor_gate::getOutputPin()
{
	return (output_pin_Y);
}

void xor_gate::dump() const
{
